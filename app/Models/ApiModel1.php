...
class ApiModel extends Model
{
    ...
    // update trupti 06-12-2022
    public function insert_edit_sale_invoice($post)
    {
        ...
        // $builder = $db->table('sales_invoice');
        // $builder->select('*');
        // $builder->where(array("other" => $post['custom_inv_no'], "is_delete" => 0, "is_cancle" => 0));
        // $builder->limit(1);
        // $result1 = $builder->get();
        ...
        //custom invoice number format change update 31-01-2023
    ...
    // update trupti 06-12-2022
    public function insert_edit_sale_return($post)
    {
        ...
        if ($post['platform_prefix'] == 'AI') {
            if (!empty($post['account_name'])) {
                $getaccount = $gmodel->get_api_data_table($post['database'], 'account', array('id' => $post['account_name']), 'name,tds_limit,state,gst,id');

                $post['tds_limit'] = $getaccount['tds_limit'];
                $post['acc_state'] = $getaccount['state'];
                $post['gst_no'] = $getaccount['gst'];
    ...
    // update trupti 06-12-2022
    public function ecom_hsn_insert_edit_sale_return($post)
    {
        if (!@$post['hsn']) {
            $msg = array('st' => 'fail', 'msg' => "Please Select any Product");
            return $msg;
    ...
    // public function tesr(){
    //     $db = $this->db;
    //     $db->setDatabase($post['database']); 
    //     $builder = $db->table('sales_invoice');
    //     $builder->select('*');
    //     $builder->where(array("custom_inv_no" => $post['custom_inv_no']));
    //     $builder->limit(1);
    //     $result = $builder->get();
    //     $result_array = $result->getRow();
    ...
    // public function ecom_insert_edit_sale_invoice($post)
    // {   
    ...
    //             if(!empty($post['custom_inv_no'])){

    //                 $sale = $gmodel->get_api_data_table($post['database'],'sales_invoice',array('custom_inv_no' => $post['custom_inv_no']),'*');
    ...
    public function ecom_insert_edit_sale_invoice($post)
    {
        ...
        if (!empty($result_array1)) {
            if ($result_array1->id == $post['id']) {
                $msg = array('st' => 'fail', 'msg' => "Order Id Already Exist!!!");
                return $msg;
            } else {
                $db->setDatabase($post['database']);
                $builder = $db->table('sales_item');
        ...
        //echo '<pre>';Print_r($s_number);exit;
        ...
        if (!empty($result_array)) {
            ...
            if (empty($msg)) {
                ...
                // echo '<pre>';print_r($getpid);
                // echo '<pre>';print_r($delete_itemid);exit;
            ...
            if (empty($msg)) {
                ...
                //     $sale = $gmodel->get_api_data_table($post['database'], 'sales_invoice', array('custom_inv_no' => $new_custome_inv_no), '*');
    ...
    // public function ecom_hsn_insert_edit_sale_invoice($post)
    // {
    ...
    //             // $sale = $gmodel->get_api_data_table($post['database'],'sales_invoice',array('other' => $post['other']),'*');
    ...
    public function ecom_hsn_insert_edit_sale_invoice($post)
    {
        ...
        $builder = $db->table('sales_invoice');
        $select = 'MAX(id) as max_id,custom_inv_no';
       ...
       // print_r($getdata);exit;
        ...
        if(!empty($getdata->max_id))
        {
            ...
            if(!empty($getdata->custom_inv_no))
            {
                $string = $getdata['custom_inv_no'];
                $outputArr = preg_split("/\//", $string);
                $count = count($outputArr);
                $last_array = $outputArr[$count - 1];
                $int_var = (int)filter_var($last_array, FILTER_SANITIZE_NUMBER_INT);
    ...
    //update trupti 06-12-2022
    public function insert_edit_purchase_invoice($post)
    {
    ...
    public function ecom_insert_edit_purchase_invoice($post)
    {
        if (!@$post['pid']) {
            $msg = array('st' => 'fail', 'msg' => "Please Select any Product");
            return $msg;
    ...
    public function insert_edit_custom_jv($post)
    {
        $db = $this->db;
        $db->setDatabase($post['database']);
