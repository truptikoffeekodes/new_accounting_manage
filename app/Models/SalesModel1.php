            ...
            if ($post['discount'] > 0) {
                ...
                for ($i = 0; $i < count($pid); $i++) {
                    ...
                    if ($post['expence'][$i] == 0) {
                        ...
                        $final_sub = $post['qty'][$i] * $post['price'][$i] - $disc_amt - $devide_disc;
                    } else {
                        $final_sub = $post['price'][$i] - $devide_disc;
                    }
                    $total += $final_sub;
                }
            }
        ...
        if ($post['disc_type'] == '%') {
            ...
            if ($post['discount'] > 0) {
                ...
                for ($i = 0; $i < count($pid); $i++) {
                    ...
                    // print_r($final_sub);echo '<br>';
                    // print_r($devide_disc);echo '<br>';
    ...
    public function insert_edit_salesreturn($post)
    {
        ...
        if (!empty($result_array)) {
            ...
            if (empty($msg)) {
                ...
                $account_builder = $db->table('sales_item');
                $account_result = $account_builder->select('GROUP_CONCAT(item_id) as item_id')->where(array("parent_id" => $post['id'], "type" => 'return', "is_delete" => 0, "is_expence" => 1))->get();
                $getAccount = $account_result->getRow();
