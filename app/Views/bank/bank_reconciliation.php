                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-lg-0">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input class="form-control datepicker_from"  name="from"
                            ...
                            <div class="col-md-4">
                                <div class="form-group mb-lg-0">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input class="form-control datepicker_to"  name="to"
                                            placeholder="YYYY-MM-DD" type="text">
                                    </div>
                                </div>
                            </div>
        ...
        <div class="card custom-card">
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-hover table-bordered table-fw-widget recon_scroll">
                        <tbody>
                            <tr>
                                <td>
                                    <?php if(!empty($bank)){ ?>
                                    ...
                                    <a href="<?=url('Bank/Statement/').@$from.'/'.@$to.'/'.@$account_id?>"
                                        class="btn btn-primary">Statement</a>
                                    <?php }  ?>

                                    <h5 style="float:right;">Reconcilation - Balance B/F :
                                        <?=@$opening_bal > 0 ? @$opening_bal.' DR' : (@$opening_bal * -1 ).' CR'?></h5>
                                </td>
                            </tr>
                            <tr colspan="4">
                            </tr>
                            ...
                            <tbody>
                                ...
                                <tr class="base">
                                    ...
                                    <td>
                                        ...
                                        <a href="#" class="recon_month" data-name="<?=@$row['ct_id']?>" data-type="text"
                                            data-pk="<?=$row['id']?>" data-title="Enter username"></a>

                                    </td>

                                    <td><button class="btn btn-link unlink"
                                            onclick="return unlink_reconsilaion(this,'<?=$row['id']?>','<?=@$row['ct_id']?>')"></button>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="5"></td>
                                    <td><b id="tot_debit"><?=number_format(@$total['bankdebit_total'],2,'.','')?> DB</b>
                                        <input type="hidden" class="tot_deb"
                                            value="<?=number_format(@$total['bankdebit_total'],2,'.','')?>">
                                    </td>

                                    <td><b id="tot_credit"><?=number_format(@$total['bankcredit_total'],2,'.','')?>
                                            CR</b>
                                ...
                                <tr>
                                    <td colspan="5"></td>
                                    <td><b> Books: </b> </td>
                                    <td><b><?=((float)@$total['bankdebit_total'] - (float)$total['bankcredit_total'] + $opening_bal) < 0 ? ((float)@$total['bankdebit_total'] - (float)$total['bankcredit_total'] + $opening_bal) * -1  .' CR' : (float)@$total['bankdebit_total'] - (float)$total['bankcredit_total'] + $opening_bal .' DB'?></b>
                                    </td>
                                    <td></td>
                                    <td></td>
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td><b> BankBK: </b> </td>
                                    <td><b
                                            id="bankbk"><?=@$opening_bal > 0 ? @$opening_bal.' DR' : (@$opening_bal * -1 ).' CR'?></b>
                                    </td>
                                    <td></td>
                                    <td></td>
                                    <input type="hidden" name="bankbk_input" value="<?=@$opening_bal?>"></b></td>
                                    <input type="hidden" name="opening" value="<?=@$opening_bal?>"></b></td>
                                </tr>
...
function calc() {
    ...
    var bankbk = parseFloat($('#bankbk').html());
    var bankbk_input = parseFloat($('input[name="opening"]').val());

    bankbk_input += (deb - cred);

    $('input[name="bankbk_input"]').val(parseFloat(bankbk_input).toFixed(2));

    if (bankbk_input < 0) {
        $('#bankbk').html((parseFloat(bankbk_input.toFixed(2)) * -1) + ' CR');
    }

    if (bankbk_input > 0) {
        $('#bankbk').html(parseFloat(bankbk_input).toFixed(2) + ' DB');
    }

    if (bankbk_input == 0) {
        $('#bankbk').html(parseFloat(bankbk_input).toFixed(2));
    }
...
$(document).ready(function() {
    ...
    $('.recon_month').editable({
        ...
        tpl: "<input type='text' class='recon_val' >",
        ...
        success: function(data, config) {
            ...
            if (mode == 'Receipt') {
                $(this).closest(".base").find('.deb').val(amount.toFixed(2))
            }
            if (mode == 'Payment') {
                $(this).closest(".base").find('.cred').val(amount.toFixed(2))
            }
