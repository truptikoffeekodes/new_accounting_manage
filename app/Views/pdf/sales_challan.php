...
<body>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td colspan="3">Ship To : <?=@$delivery['name']?>
                    <br><?=@$account['gst_add']?>
                    <br><?=@$challan['ship_city']?> &nbsp;<?=@$challan['ship_pin']?>
                    <br><?=@$challan['ship_state']?>
                    <br><?=@$challan['ship_country']?>
                ...
                <td colspan="3">
                    <b>Challan No : <?=@$challan['challan_no']?></b><br>
                    <b>Voucher No : <?=@$challan['id']?></b>
                    <br><b>Challan Date : <?=user_date(@$challan['challan_date'])?></b>
            ...
            <tr>
                <td colspan="4">Broker :<?=@$challan['broker_name']?></td>
                <td colspan="5">Transporter Name :<?=@$transport['name']?></td>
            </tr>
            <tr>
                <td colspan="3">
                    L.R.No :<?=@$challan['lr_no']?>
                    <br>Transport :
                </td>
                <td colspan="3">L.R.Date : <?=@$challan['lr_date']?>
                    <br>Transport Type : <?=@$challan['transport_mode']?>
                </td>
                <td colspan="3">Weight :<?=@$challan['weight']?>
                    <br>Freight :<?=@$challan['freight']?>
                </td>
    ...
    <?php
                if($total != 0 ){
                    if ($challan['disc_type'] == '%') {
                        $discount_amount = ($total * ($challan['discount'] / 100));
                        $disc_avg_per = $discount_amount / $total;
                    } else {
                        $disc_avg_per = ($challan['discount'] ? (float)$challan['discount'] : 0) / $total ? $total : 0;
                    }
                
                    if ($challan['amty'] > 0) {
                        if ($challan['amty_type'] == '%') {
                            $amty_amount = ($total * ($challan['amty'] / 100));
                            $add_amt_per = $amty_amount / $total;
                        } else {
                            $add_amt_per = $challan['amty'] / $total;
                        }
                    } else {
                        $add_amt_per = 0;
                    }
    ...
    <table class="T-border2" style="width:100%">
        ...
        <tr>
            ...
            <td style="border-right: 1px solid black; border-top: 1px solid black;text-align: right;"> <b><?=@$challan['round_diff']?></b><br>
            </td>
