...
<body>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td colspan="3">Ship To : <?=@$delivery['name']?>
                    <br><?=@$account['gst_add']?>
                    <br><?=@$general['ship_city']?> &nbsp;<?=@$general['ship_pin']?>
                    <br><?=@$general['ship_state']?>
                    <br><?=@$general['ship_country']?>
                ...
                <td colspan="2">
                    <b>Invoice No : <?=@$general['supp_inv']?></b><br>
                    <b>Voucher No : <?=@$general['invoice_no']?></b>
                    <br><b>Invoice Date : <?=user_date(@$general['doc_date'])?></b>
            ...
            <tr>
                <td colspan="4">Broker :<?=@$general['broker_name']?></td>
                <td colspan="4">Transporter Name :<?=@$transport['name']?></td>
            </tr>
            <tr>
                <td colspan="3">
                    L.R.No :<?=@$general['lr_no']?>
                    <br>Transport :
                </td>
                <td colspan="3">L.R.Date : <?=@$general['lr_date']?>
                    <br>Transport Type : <?=@$general['transport_mode']?>
                </td>
    ...
    <?php
                if($total != 0 ){
                    if ($general['disc_type'] == '%') {
                        $discount_amount = ($total * ($general['discount'] / 100));
                        $disc_avg_per = $discount_amount / $total;
                    } else {
                        $disc_avg_per = ($general['discount'] ? (float)$general['discount'] : 0) / ($total ? $total : 0);
                    }
                    if ($general['amty'] > 0) {
                        if ($general['amty_type'] == '%') {
                            $amty_amount = ($total * ($general['amty'] / 100));
                            $add_amt_per = $amty_amount / $total;
                        } else {
                            $add_amt_per = $general['amty'] / $total;
                        }
                    } else {
                        $add_amt_per = 0;
                    }
                ...
                //ssprint_r($total);
                    for ($i = 0; $i < count($acc); $i++) {
                       ...
                       //echo 'before'; print_r($total);

                         $total += $final_tot;
             
                       //echo 'in'; print_r($total);
