<?= $this->extend(THEME . 'templete') ?>

<?= $this->section('content') ?>

<!-- Page Header -->
<div class="page-header">

    <div>
        <h2 class="main-content-title tx-24 mg-b-5"><?=$title?></h2>
        <ol class="breadcrumb">
            <li class="breadcrumb-item"><a href="#">Dashboard</a></li>
            <li class="breadcrumb-item active" aria-current="page"><?=$title?></li>
        </ol>
    </div>

    <div class="btn btn-list">
        <a href="<?= url('purchase/add_general_pur/general') ?>" class="btn ripple btn-primary"><i
                class="fe fe-external-link"></i>
            Add New</a>
        <a href="<?= url('purchase/add_general_pur/return') ?>" class="btn ripple btn-danger"><i
                class="fe fe-external-link"></i>
            Add New Return</a>
    </div>
</div>
<!-- End Page Header -->
<div class="row">
    <div class="col-lg-12">
        <div class="card custom-card main-content-body-profile">
            <div class="card-header card-header-divider">
                <nav class="nav main-nav-line">
                    <a class="nav-link active" data-toggle="tab" href="#general">General</a>
                    <a class="nav-link" data-toggle="tab" href="#return_div">Return</a>
                    <!-- <a class="nav-link" data-toggle="tab" href="#auction">Date</a>
                         -->

                </nav>

                <div class="card-body tab-content h-100">
                    <div class="tab-pane active" id="general">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-fw-widget" id="table_list_data"
                                data-id="general_purchase" data-module="purchase" data-filter_data='general'>
                                <thead>
                                    <tr>
                                        <th>Id</th>
                                        <th>Voucher Type</th>
                                        <th>Party Account</th>
                                        <th>Supplier Invoice</th>
                                        <th>Total Amount</th>
                                        <th>Status</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
                                <tfoot>
                                    <th>Id</th>
                                    <th>Voucher Type</th>
                                    <th>Party Account</th>
                                    <th>Supplier Invoice</th>
                                    <th>Total Amount</th>
                                    <th>Status</th>
                                    <th>Action</th>
                                </tfoot>
                            </table>

                        </div>

                    </div>

                    <div class="tab-pane" id="return_div">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover table-fw-widget" id="table_list_data1"
                                data-id="general_purchase" data-module="purchase" data-filter_data='return'
                                style="width: inherit">
                                <thead>
                                    <tr>
                                        <th>Id</th>
                                        <th>Voucher Type</th>
                                        <th>Party Account</th>
                                        <th>Supplier Invoice</th>
                                        <th>Total Amount</th>
                                        <th>Status</th>
                                        <th>Action</th>
                                    </tr>
                                </thead>
                                <tbody>
                                </tbody>
                                <tfoot>
                                    <th>Id</th>
                                    <th>Voucher Type</th>
                                    <th>Party Account</th>
                                    <th>Supplier Invoice</th>
                                    <th>Total Amount</th>
                                    <th>Status</th>
                                    <th>Action</th>
                                </tfoot>
                            </table>

                        </div>
                    </div>





                </div>


            </div>
        </div>

    </div>

</div>

<?= $this->endSection() ?>

<?= $this->section('scripts') ?>

<script type="text/javascript">
$(document).ready(function() {
    datatable_load('');
});

function datatable_load(filter_val) {

    $("#table_list_data").DataTable({
        "destroy": true,
        "processing": true,
        "serverSide": true,
        "autoWidth": false,
        "order": [
            [0, "desc"]
        ],
        "dom": "<'row be-datatable-header'<'col-sm-2'l><'col-sm-6 text-left'B><'col-sm-4 text-right'f>>" +
            "<'row be-datatable-body'<'col-sm-12'tr>>" +
            "<'row be-datatable-footer'<'col-sm-5'i><'col-sm-7'p>>",
        "buttons": [
            'copy', 'csv', 'excel', 'pdf'
        ],
        "ajax": {
            "type": "POST",
            "url": PATH + "/" + $("#table_list_data").data('module') + "/GetData/" + $("#table_list_data").data(
                'id') + '?filter_data=' + $("#table_list_data").data('filter_data')
        },
        "lengthMenu": [
            [10, 25, 50, 100, 200, 300, -1],
            [10, 25, 50, 100, 200, 300, "All"]
        ],
        "footerCallback": function(row, data, start, end, display) {

        }
    });


    $("#table_list_data1").DataTable({
        "destroy": true,
        "processing": true,
        "serverSide": true,
        "autoWidth": false,
        "order": [
            [0, "desc"]
        ],
        "dom": "<'row be-datatable-header'<'col-sm-2'l><'col-sm-6 text-left'B><'col-sm-4 text-right'f>>" +
            "<'row be-datatable-body'<'col-sm-12'tr>>" +
            "<'row be-datatable-footer'<'col-sm-5'i><'col-sm-7'p>>",
        "buttons": [
            'copy', 'csv', 'excel', 'pdf'
        ],
        "ajax": {
            "type": "POST",
            "url": PATH + "/" + $("#table_list_data1").data('module') + "/GetData/" + $("#table_list_data1")
                .data(
                    'id') + '?filter_data=' + $("#table_list_data1").data('filter_data')
        },
        "lengthMenu": [
            [10, 25, 50, 100, 200, 300, -1],
            [10, 25, 50, 100, 200, 300, "All"]
        ],
        "footerCallback": function(row, data, start, end, display) {

        }
    });

}


function editable_remove(data_edit) {
    //alert("hcgejh");
    var type = 'Remove';

    var data_val = $(data_edit).data('val');

    var ot_title = $(data_edit).attr('title');
    var pkno = $(data_edit).data('pk');
    swal.fire({
        title: "Are you sure Remove " + ot_title + " ?",
        text: "You will not be able to recover this imaginary file!",
        type: "warning",
        showCancelButton: true,
        confirmButtonClass: "btn-danger",
        confirmButtonText: "Yes, delete it!",
        cancelButtonText: "No, cancel pls!",

    }).then((result) => {
        // function(isConfirm) {
        if (result.value) {
            _data = $.param({
                pk: pkno
            }) + '&' + $.param({
                val: data_val
            }) + '&' + $.param({
                type: type
            }) + '&' + $.param({
                method: $("#table_list_data").data('id')
            });

            if (data_val != undefined && data_val != '') {
                $.post(PATH + "/" + $("#table_list_data").data('module') + "/Action/Update", _data, function(
                    data) {
                    if (data.st == 'success') {
                        datatable_load('');
                        swal.fire("Deleted!", "Your imaginary file has been deleted.", "success");
                    }
                });
            }

        } else {
            swal("Cancelled", "Your imaginary file is safe :)", "error");
        }
        // })}
    });
}


function editable_os(data_edit) {
    var type = 'Cancle';
    var data_val = $(data_edit).data('val');
    var data_method = $(data_edit).data('method');
    var str = 'Do You Want to Cancle This Invoice..!!';
    var ot_title = $(data_edit).attr('title');
    var pkno = $(data_edit).data('pk');
    var select_input = {
        "1": "Yes",
        "0": "No"
    };

    swal({
        title: ot_title,
        text: str,
        type: "warning",
        showCancelButton: true,
        confirmButtonClass: "btn-danger",
        confirmButtonText: "Yes, delete it!",
        cancelButtonText: "No, cancel pls!",
    }).then(function(result) {

        _data = $.param({
            pk: pkno
        }) + '&' + $.param({
            val: result.value
        }) + '&' + $.param({
            type: type
        }) + '&' + $.param({
            method: $("#table_list_data").data('id')
        });

        if (result.value != undefined && result.value != '') {
            $.post(PATH + "/Purchase/Action/Update", _data, function(data) {

                if (data.st == 'success') {
                    datatable_load('');
                    var selectdata = result.value;
                    $(data_edit).data('val', selectdata);
                    $(data_edit).html(select_input[selectdata]);
                    swal("success!", "Challan Update successfully!", "success");
                }

            });
        }
    });
}


</script>

<?= $this->endSection() ?>