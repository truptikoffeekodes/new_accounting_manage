...
<div class="row">
    <div class="col-lg-12">
        <form class="ajax-form-taka" method="post" action="<?=url('Milling/insert_SaleTaka')?>">
                ...
                <table class="table table-bordered mg-b-0">
                    ...
                    {   
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="qty[<?=@$taka[$i]['taka_no']?>]"  readonly class="form-control rec_qty" value="<?=@$taka[$i]['received_qty']?>" onkeyup="calc()"></td>

                            <td style="text-align:center;"><input type="checkbox" class="checkItem" name="check[]" onclick = "calc_qty(this)" value="<?=@$taka[$i]['taka_no']?>" <?=(@$taka[$i]['is_sale']==1) ? 'checked' : ''?> ></td>
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.st == 'success') {
                ...
                $('.' + tr).closest("tr").find('input[name="saleMillTaka_ids[]"]').val(response.saleMillTaka_ids);
                calculate();
            } else {
                $('.form_proccessing').html('');
                $('#save_data').prop('disabled', false);
