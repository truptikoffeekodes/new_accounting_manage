...
<div class="row">
    <div class="col-lg-12">
        <form class="ajax-form-taka" method="post" action="<?=url('Milling/insert_MillRectaka')?>">
            ...
            <div class="table-responsive">
                <table class="table table-bordered mg-b-0">
                    ...
                    {   
                        ...
                        <tr>
                            <td><?=$j++?></td>
                            <input type="hidden" name="taka_id[]" value="<?=$mill_taka[$i]['id']?>">
                            <input type="hidden" name="mill_item_id[]" value="<?=$mill_taka[$i]['mill_item_id']?>">
                            <input type="hidden" name="main_taka" id="main_taka" value="">
                            <input type="hidden" name="millRecTakaID[]" value="<?=@$mill_taka[$i]['millRecTakaID']?>">

                            <td><input type="text" name="taka_no[]" value="<?=$mill_taka[$i]['taka_no']?>" readonly
                            ...
                            <td><input type="text" name="quantity[]" onkeypress="return isNumberKey(event)"
                                    readonly placeholder="Enter Quantity"
                                    value="<?=$mill_taka[$i]['quantity']?>" class="form-control"></td>
                            
                            <td><input type="text" name="rec_qty[]" class="form-control rec_qty" value="<?=@$mill_taka[$i]['received_qty']?>" <?=@$mill_taka[$i]['disabled'] == 1 ? 'readonly' : '' ?> onkeyup="calc()"></td>

                            <td><input type="text" name="taka_cut[]" onkeypress="return isNumberKey(event)"
                                    placeholder="" readonly value="<?=@$mill_taka[$i]['cut']?>"
                                    class="form-control"></td>
                        </tr>
                        <?php
                        }
                    ...
                    <tfooter>
                        <tr>
                            <td colspan="2"> </td>
                            <td>Taka TP </td>
                            <td><input class="form-control" id="taka_tp" readonly></td>
...
function calc(){
   ...
   var total_cut = 0;
    ...
    $('.rec_qty').keypress(function(e) {
        if (e.which == 13) {
            $(this).closest('tr').next().find('input.rec_qty').focus();
            e.preventDefault();
        }
    });
...
$('.ajax-form-taka').on('submit', function(e) {
    ...
    $('.' + tr).closest("tr").find('input[name="rec_meter[]"]').val(total_qty);

    var taka_tp =  $('#taka_tp').val();
    $('.' + tr).closest("tr").find('input[name="taka_tp[]"]').val(taka_tp);

    var cut =  $('#total_cut').val();
    $('.' + tr).closest("tr").find('input[name="rec_cut[]"]').val(cut);

    var pcs =  $('#main_taka').val();
    $('.' + tr).closest("tr").find('input[name="rec_taka[]"]').val(pcs);
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.st == 'success') {
                ...
                $('.' + tr).closest("tr").find('input[name="MillRecTB_id[]"]').val(response.MillRecTB_id);
                $('.' + tr).closest("tr").find('input[name="MillTakaTB_id[]"]').val(response.milltaka_id);
