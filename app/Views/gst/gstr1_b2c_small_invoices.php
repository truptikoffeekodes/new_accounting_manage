...
<div class="page-header">
    ...
    <div class="btn btn-list">
        ...
        <a href="<?=url('gst/b2c_small_xls_export?from='.$start_date.'&to='.$end_date.'&type='.@$type)?>"
...
<div class="responsive-background">
    <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <div class="advanced-search">
            <?php if($type == 'sales'){ ?>
            <form method="get" action = "<?=url('Gst/b2c_small_sales_inv_vouchers')?>">
            <?php }else{ ?>
            <form method="get" action = "<?=url('Gst/b2c_small_gnrl_sales_inv_vouchers')?>">
            <?php } ?>
                        ...
                        <div class="card-header" id="headingOne" role="tab">
                            <a aria-controls="collapseOne" aria-expanded="false" data-toggle="collapse"
                                href="#collapseOne" class="collapsed">Total Voucher<label
                                    style="float:right;"><?=($type == 'sales') ? @$sale_b2c_small['count'] :$gnrl_sale_b2c_small['count'] ?></label>
                            </a>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover table-fw-widget" id="table_list_data" data-id=""
                        ...
                        <tbody>
                            ...
                            <?php 
                            if($type == 'sales'){
                                $result = $sale_b2c_small;
                            }else{
                                $result = $gnrl_sale_b2c_small;
                            }
                            foreach($result['data'] as $row ) { ?>
                            <tr>
                                <th><?=$row['invoice_no']?></th>
                                <th><?=@$row['custom_inv_no']?></th>
                                <th><?=user_date($row['invoice_date'])?></th>

                                <?php if($type == 'sales'){ ?>    
                                <td><a href="<?=url('sales/add_salesinvoice/'.$row['id'])?>"><?=$row['name']?></a></td>
                                <?php }else{ ?>
                                <td><a href="<?=url('sales/sales/add_ACinvoice/general/'.$row['id'])?>"><?=$row['name']?></a></td>
                                <?php } ?>
