...
<div class="row">
    <div class="col-lg-12">
    <form action="<?=url('Master/add_class')?>" class="ajax-form-submit" method="post"
            enctype="multipart/form-data">
            <div class="form-group">
                <label class="form-label">Name: <span class="tx-danger">*</span></label>
                <input class="form-control" required  placeholder="Name" onkeyup="code_generate(this.value)" name="name" value="<?=@$class['name']?>" type="text">
                <input name="id" value="<?=@$class['id']?>" type="hidden">
            </div>
            <div class="form-group">
                <label class="form-label">Code: <span class="tx-danger">*</span></label>
                <input class="form-control" required  placeholder="Code" id="code"  name="code" value="<?=@$class['code']?>" type="text">
                
            </div>
            <div class="form-group">
                <label class="form-label">Description: <span class="tx-danger">*</span></label>
                <textarea class="form-control" required placeholder="Description" name="description" 
                    type="text"><?=@$class['description']?></textarea>
            </div>
            <!-- <div class="form-group">
                <label class="form-label">Status: <span class="tx-danger">*</span></label>
                <select class="form-control" name="status">
                    <option label="Select Status" disable>
                    </option>
                    <option <?= ( @$class['status'] == "1" ? 'selected' : '' ) ?> value="1">
                    Active
                    </option>
                    <option <?= ( @$class['status'] == "0" ? 'selected' : '' ) ?> value="0">
                    InActive
                    </option>
                </select>
            </div> -->
